...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   aidlc engine orchestrate next --stage environment-provisioning --single
   ```
...
3. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   aidlc engine orchestrate report --single --stage environment-provisioning --result completed
   ```
